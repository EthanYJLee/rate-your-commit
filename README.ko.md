...
- [x] **0.0.1** — 화면 설계, 아키텍처 설계, 라이선스/거버넌스, README, 초기 저장소 스캐폴딩
- [x] **v1.0** — 아이덴티티 매핑 + Git/이슈 연동 + 개인 스코어카드 (GitHub 커넥터)
- [x] **v1.1** — GitLab / Jira / Linear 커넥터 추가
- [x] **v1.2** — 이메일/비밀번호 로그인, 역할 기반 접근제어(admin/member), CSRF 방어 (현재 버전)
- [x] **v1.3** — 팀 계층 구조, 대시보드 리스크 알림, 보상등급 확정(finalize) 워크플로우
- [ ] v2.0 — 동료평가(360도) 모듈
- [ ] v2.1 — 프로젝트 단위 성과 집계 화면, 보상등급 2단계 승인 워크플로우(PM→인사)
